...
def correct_lineage_and_umi(
    ...
    for i in range(n_iter):
        ...
        # 1) collapse lineage_bc within each UMI, length-aware (HD=1 rule)
        out["__bc_len__"] = out["lineage_bc_corr"].str.len()
        bc_parent_total = {}
        for blen, sub in tqdm(out.groupby(["__bc_len__"]), desc="Collapsing barcodes (HD=1)", leave=True):
            cnt = Counter(dict(sub.groupby("lineage_bc_corr")[count_col].sum()))
            parent = collapse_directional(cnt.items())
        ...
        # 2) collapse UMIs within each lineage_bc (HD=1 rule)
        parent_all = {}
        for bc_val, sub in tqdm(out.groupby("lineage_bc_corr"), desc="Collapsing UMIs (HD=1)", leave=True):
            cnt = Counter(dict(sub.groupby("umi_corr")[count_col].sum()))
            parent = collapse_directional(cnt.items())
            parent_all.update(parent)
        ...
        # 3) collapse barcodes within each UMI (HD <= max(0.01 * length, 2))
        parent_lenaware = {}
        # group by UMI and barcode length so Hamming distance is well-defined
        for (umi_val, blen), sub in tqdm(
            out.groupby(["umi_corr", "__bc_len__"]),
            desc="Collapsing barcodes (length-aware HD)",
            leave=True
        ):
            if blen == 0:
                continue
            ...
            if not cnt:
                continue
            # threshold: at least 2, else 1% of length rounded
            hd_thresh = max(int(round(0.01 * blen)), 2)
            parent = collapse_within_hd(cnt.items(), max_hd=hd_thresh)
            parent_lenaware.update(parent)

        before = out["lineage_bc_corr"].ne(out["lineage_bc_corr"].map(lambda b: parent_lenaware.get(b, b))).sum()
        out["lineage_bc_corr"] = out["lineage_bc_corr"].map(lambda b: parent_lenaware.get(b, b))
        total_bc_merges += int(before)
...
def main():
    ...
    try:
        ...
        # Diagnostic plot 1: Distribution of lineage barcode lengths (before filtering)
        logger.info("Generating diagnostic plots...")
        plot_barcode_length_distribution(results_df, sample_output_dir, title_suffix="", unedited_bc_len=UNEDITED_BC_LEN)
        ...
        # Diagnostic plot 2: Distribution of read counts
        plot_reads_cutoff_distribution(results_df, args.reads_cutoff, sample_output_dir)
        
        # Diagnostic plot 3: Reads cutoff vs Number of UMIs
        plot_cutoff_vs_num_umis(results_df, args.reads_cutoff, sample_output_dir)
        
        # Diagnostic plot 4: Reads cutoff vs Fraction of reads retained
        plot_cutoff_vs_fraction_reads_retained(results_df, args.reads_cutoff, sample_output_dir)
        ...
        logger.info(f"Denoising stats: {stats}")
        ...
        plot_clone_size_distribution(final, sample_output_dir)
        
        # Diagnostic plot 8: Distribution of lineage barcode lengths by editing events
        plot_barcode_length_by_editing_events(final, sample_output_dir, unedited_bc_len=UNEDITED_BC_LEN)
        
        # Step 8: Save output
        output_file = os.path.join(sample_output_dir, f'{args.sample_id}_alleles.csv')
        final2.to_csv(output_file, index=False)
        logger.info(f"Results saved to: {output_file}")
        logger.info(f"Total unique alleles: {len(final2)}")
        logger.info(f"Diagnostic plots saved to: {sample_output_dir}")
